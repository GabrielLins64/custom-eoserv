...
namespace Commands
{
	...
	void SetX(const std::vector<std::string> &arguments, Command_Source *from, std::string set)
	{
		...
		else
		{
			...
			if (set == "level")
				...
				(stats = true, victim->tp) = std::min(std::max(util::to_int(arguments[1]), 0), int(from->SourceWorld()->config["MaxStat"]));
			else if (set == "int")
	...
	Register({"settp", {"victim", "value"}, {}, 4}, std::bind(SetX, _1, _2, "tp"), CMD_FLAG_DUTY_RESTRICT);
